...
class _ShippingAddressState extends State<ShippingAddress> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.endOfFrame.then((_) async {
      ...
      final addressValue = await Provider.of<CartModel>(
        context,
        listen: false,
      ).getAddress();
    ...
    if (currentCountry.isNotEmpty) {
      try {
        if (countries.isEmpty) {
          try {
            country = picker.CountryPickerUtils.getCountryByIsoCode(
              currentCountry,
            );
            address.country = country.isoCode;
            countryName = country.name;
          } catch (_) {
            country = picker.CountryPickerUtils.getCountryByName(
              currentCountry,
            );
            address.country = country.isoCode;
            countryName = country.name;
    ...
    return List.generate(_fieldPosition.length, (index) {
        ...
        return Padding(
          ...
          child: ButtonTheme(
            ...
            child: ElevatedButton(
              ...
              onPressed: () async {
                if (ServerConfig().isShopify && user?.isSocial != true) {
                  final selectedAddress =
                      await Navigator.of(
                            context,
                          ).pushNamed(RouteList.addressBook, arguments: address)
                          as Address?;
                  if (selectedAddress != null) {
                    updateAddress(selectedAddress);
                    _onNext(ignoreFormValidation: true);
                  }
                  return;
                }
      ...
      if (currentFieldType == AddressFieldType.phoneNumber &&
        ...
        return InternationalPhoneNumberInput(
          ...
          onSaved: (value) {
            onTextFieldSaved(value.phoneNumber, currentFieldType);
          },
      ...
      return Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            onSaved: (value) => onTextFieldSaved(value, currentFieldType),
          ),
        ],
      );
    });
